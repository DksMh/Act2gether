...
function initializeEventListeners() {
  ...
  requiredTerms.forEach((term) =>
    term.addEventListener("change", updateAgreeAll)
  );
...
function showAgeRestrictionNotice() {
  ...
  if (!notice) {
    ...
    notice.innerHTML =
      "⚠️ 죄송합니다. 본 서비스는 50세 이상 액티브 시니어를 위한 서비스입니다.";
...
function updateAgreeAll() {
  ...
  let allRequiredChecked = Array.from(requiredTerms).every(
    (term) => term.checked
  );
  let allOptionalChecked = Array.from(optionalTerms).every(
    (term) => term.checked
  );
...
function updateNextButton() {
  ...
  const allRequiredChecked = Array.from(requiredTerms).every(
    (term) => term.checked
  );
        ...
        [
          ...
          if (element) {
            ...
            element.style.cursor =
              element.tagName === "SELECT" ? "pointer" : "text";
          }
        });
