...
async function renderEnrolled() {
  ...
  rows.forEach((item) => {
    ...
    const badgeText =
      d > 0
        ? `여행 시작까지 <strong style="color:#ff5900">${d}</strong>일 남았어요!`
        : d === 0
    ...
    card.innerHTML = `
      ...
      <div class="meta">
        <span>${(item.startDate || "").replaceAll("-", ".")} ~ ${(
      item.endDate || ""
    ).replaceAll("-", ".")}</span>
        <span>${Number(item.maxMembers) || 0}명 모집 (현재 ${
      Number(item.currentMembers) || 0
    }명)</span>
      </div>
    `;
    card.addEventListener("click", () => {
      if (item.groupId)
        location.href = `/community?groupId=${encodeURIComponent(
          item.groupId
        )}`;
    });
  ...
  function updateNav() {
    const canScroll =
      enableScroll && track.scrollWidth > viewport.clientWidth + 2;
    prevBtn.hidden = nextBtn.hidden = !canScroll;
    prevBtn.disabled = viewport.scrollLeft <= 0;
    nextBtn.disabled =
      viewport.scrollLeft + viewport.clientWidth >= track.scrollWidth - 1;
  }
...
function buildLocalCard(item) {
  ...
  const pillText =
    n > 0
      ? `여행 시작까지 <span class="num">${n}</span>일 남았어요!`
      : n === 0
      ? `<span class="num">오늘</span> 출발!`
      : `여행 중이거나 종료`;
...
async function renderLocal() {
  ...
  try {
    const res = await fetch(
      `/community/gathering/${encodeURIComponent(region)}`,
      {
        method: "GET",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  const hideRest = () =>
    cards.forEach((c, i) => {
      c.style.display = i < CARDS_TO_SHOW ? "" : "none";
    });
  const showAll = () =>
    cards.forEach((c) => {
      c.style.display = "";
